...
class UpdateProfileFormValidator
{
    private $validation;
    private $data;
    private $profileId;

    public function __construct($data, $profileId)
    {
    ...
    {
        $this->validation->isValid($this->data['email'],
        ...
        foreach ($this->data['email'] as $email) {
            array_push($emailValidators, new UniqueValidator("Такой email: $email занят",
                'emails', 'title', $this->profileId, 'profile_id'));
    ...
    private function validateTelephone()
    {
        ...
        foreach ($this->data['telephone'] as $telephone) {
            array_push($telephoneValidators, new UniqueValidator("Такой телефон: $telephone занят",
                'telephones', 'title', $this->profileId, 'profile_id'));
